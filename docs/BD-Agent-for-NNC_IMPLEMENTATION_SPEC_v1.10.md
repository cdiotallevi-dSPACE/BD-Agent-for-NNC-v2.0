...
**Design document version:** `1.10.0`  
**Runtime compatibility tag:** `1.10`  
**Date:** `2026-08-05`
...
## Version 1.9 — Product applicability scoring is independent of current ONNX/code-generation adoption

**Historical transition note:** this section is retained for design history. Its 20-point mapping model is superseded by Version 1.10.

### 1.9.1 Normative precedence

This Version 1.9 section supersedes any earlier Version 1.3–1.8 rule that sets
`mapping_eligible: false` or `score_contribution: 0` solely because ONNX,
generated C/C++, or an existing model-to-code workflow is not publicly proven.

The BDA is a business-development applicability assessor. Its main scoring
question is:

> **Could Neural Net Coder reasonably be applied to implement this
> target-company neural solution on the described embedded/onboard target?**

That question is different from:

> Does public evidence prove that the company already uses ONNX, generated C,
> Neural Net Coder, or an equivalent deployment workflow?

The first question drives Product Match. The second drives deployment readiness,
qualification questions, and sales risk.

### 1.9.2 Three separate decision dimensions

```yaml
decision_dimensions:
  technical_applicability:
    affects_overall_product_match: true
    question: Could NNC reasonably implement the neural solution?

  deployment_readiness:
    affects_overall_product_match: false
    reported_separately: true
    question: Are model export, ONNX/operator support, target hardware and resource constraints already known?

  current_tool_adoption:
    affects_overall_product_match: false
    reported_as_sales_context: true
    question: Does public evidence show that the company already uses NNC or an equivalent model-to-code workflow?
```

Absence of current ONNX/code-generation evidence SHALL NOT automatically reduce
a technically strong NNC applicability result to zero.

### 1.9.3 NNC relevance classes

```yaml
nnc_relevance_class:
  nnc_deployment_evidence:
    meaning: Neural solution plus embedded target plus deployment workflow are directly confirmed.
    mapping_eligible: true
    score_band: very_high

  nnc_applicability_evidence:
    meaning: Neural solution/component and embedded/onboard application are confirmed; NNC is a credible implementation option although ONNX/code generation are not yet proven.
    mapping_eligible: true
    score_band: high

  nnc_opportunity_evidence:
    meaning: Applied ML activity is relevant, but neural or embedded applicability remains materially uncertain.
    mapping_eligible: conditional_or_false
    score_band: low_to_medium
...
### 1.9.4 Strong inferred applicability gate

A source or tightly related patent/application family SHALL qualify as
`nnc_applicability_evidence` when all three groups below pass:

```yaml
strong_inferred_applicability_gate:
  company_attribution:
    required: established

  neural_solution:
    any_of:
      - confirmed_neural_model
      - confirmed_neural_estimator
      - confirmed_neural_classifier
      - confirmed_neural_learning_agent
      - confirmed_hybrid_solution_with_explicit_neural_component
      - patent_claimed_neural_embodiment_directly_tied_to_the_engineering_solution

  embedded_or_onboard_target:
    any_of:
      - MCU
      - ECU
      - programmable_electronic_control_unit
      - dedicated_electronic_control_unit
      - automotive_controller
  ...
  engineering_function:
    any_of:
      ...
      - soft_sensor
      - state_estimation
      - latent_variable_estimation
      - condition_recognition
      - predictive_control_support
      - anomaly_detection
      - prognostics
      - learned_calibration
      - learned_observer
      - neural_control_component
```

Passing result:

```yaml
relevance_class: nnc_applicability_evidence
mapping_eligible: true
applicability_mapping_type: strong_inferred_applicability
```

This is an engineering-fit inference. It SHALL NOT be worded as proof that the
customer already uses NNC, ONNX, or generated C code.

### 1.9.5 ONNX and generated code move from applicability gates to qualification/readiness

```yaml
ONNX_status:
  confirmed:
    deployment_readiness_effect: positive
  unconfirmed:
    technical_applicability_effect: no_automatic_rejection
    deployment_readiness_effect: reduce
    qualification_question_required: true
  explicitly_incompatible:
    technical_applicability_effect: reduce_or_block_after_operator_model_analysis

generated_code_workflow:
  confirmed:
    deployment_readiness_effect: positive
  unconfirmed:
    technical_applicability_effect: no_automatic_rejection
...
The BDA SHALL ask whether the trained neural component can be exported or
converted to an NNC-supported ONNX graph. It SHALL not assume that this is
already true.

### 1.9.6 Deterministic applicability contribution bands

```yaml
applicability_mapping_score_bands:
  direct_deployment_fit:
    contribution: 25

  strong_inferred_applicability:
    contribution: 20

  moderate_inferred_applicability:
    contribution: 10
...
`strong_inferred_applicability` is the normal high-score path for a public
company neural use case that is intended for an embedded/onboard controller but
does not yet disclose ONNX or automated code generation.

### 1.9.7 Use-case-level caps and deduplication

The score SHALL measure distinct engineering opportunities, not document count.

```yaml
use_case_score_caps:
  per_use_case_family_maximum: 25
  per_distinct_engineering_application_maximum: 25
  corroborating_source_bonus_maximum: 5
```

Multiple documents supporting the same engineering application increase
confidence but SHALL not each add a full 20–25 points.

Distinct applications MAY contribute independently. For example:

- tractive-limit / wheel-stability estimation;
- predictive battery charging / virtual battery measurements.

### 1.9.8 Separate deployment-readiness score

```yaml
deployment_readiness_score:
  range: 0..100
  included_in_overall_product_match: false
  components:
    ONNX_export_confirmed: 25
    supported_operator_set_confirmed: 15
    target_processor_identified: 15
    RAM_ROM_latency_constraints_known: 15
    existing_C_CPP_integration_workflow: 10
    model_verification_workflow_confirmed: 10
    safety_process_or_MISRA_need_confirmed: 10
```

An unconfirmed item contributes zero readiness points but SHALL NOT become a
negative Product Match penalty.

A valid BDA result may therefore be:

```yaml
overall_product_match: high
deployment_readiness: low_or_unknown
```

### 1.9.9 Applicability mapping language

A `strong_inferred_applicability` mapping SHOULD use wording such as:

> The target-company neural solution is technically suitable for an NNC
> implementation workflow, subject to qualification of model export/ONNX
> compatibility, operator support, target hardware, and resource constraints.

It SHALL NOT say:

- the company already uses Neural Net Coder;
- the company already uses ONNX;
- the company already generates C code from the model;
- a patent's optional neural embodiment is the confirmed production
  implementation;

unless the public evidence explicitly supports the statement.

### 1.9.10 NNC capabilities that may be mapped for strong inferred fit

When the strong inferred applicability gate passes, the BDA MAY infer these
NNC-relevant engineering capabilities as applicable to the solution:

```yaml
nnc_capabilities_for_strong_inferred_fit:
  - trained_model_handover_via_ONNX
  - deterministic_C_code_generation
  - static_memory_implementation
  - resource_forecast_RAM_ROM_stack_execution_time
  - post_training_optimization_and_quantization
  - back_to_back_verification_ONNX_vs_generated_code
  - embedded_target_integration_support
```

These are potential dSPACE capabilities for solving the implementation need,
not proof of capabilities currently used by the target company.

### 1.9.11 Patent-specific decision rule

A target-company patent SHALL receive a high NNC applicability contribution
when it establishes:

1. a neural model, neural estimator, neural classifier, neural learning agent,
   explicit hybrid neural component, or explicit claimed neural embodiment
   directly tied to the described solution;
2. an onboard/embedded controller or real-time vehicle/device context; and
3. an NNC-relevant engineering function.
...
mapping_eligible: true
applicability_mapping_type: strong_inferred_applicability
default_score_contribution: 20
ONNX_status: unconfirmed_allowed
current_code_generation_workflow: unconfirmed_allowed
deployment_readiness: partial_or_unknown
```

A generic neural-network mention unrelated to the patented function remains
`nnc_opportunity_evidence` and does not automatically pass this rule.

### 1.9.12 Required GM regression: both patents must score as high applicability

The known publication numbers and URLs are regression assertions only. Discovery
continues to follow Version 1.8 patent-native assignee enumeration and SHALL NOT
use them as production seeds.

```yaml
applicability_scoring_regression:
  fixture_id: GM-NNC-HIGH-APPLICABILITY-DUAL-PATENT
  target_company: General Motors

  patents:
    - publication_number: US12246700B2
      engineering_application: tractive_limit_and_wheel_stability
      evidence:
        company_attribution: established
        ML_regression: confirmed
        ML_classification: confirmed
        neural_network_embodiment: explicit_claimed_option
        onboard_or_ECU_context: confirmed
      expected:
        relevance_class: nnc_applicability_evidence
        mapping_eligible: true
        mapping_type: strong_inferred_applicability
        technical_applicability: high
        score_contribution: 20
        ONNX_status: unconfirmed
        current_code_generation_workflow: unconfirmed
        deployment_readiness: partial_or_unknown
        qualification_required: true

    - publication_number: US20240302440A1
      engineering_application: predictive_battery_charging_and_virtual_measurements
      evidence:
        company_attribution: established
        neural_estimator_embodiment: confirmed
        neural_learning_agent: confirmed
        hybrid_physics_data_driven_estimation: confirmed
        onboard_BMS_or_charging_controller_context: confirmed
      expected:
        relevance_class: nnc_applicability_evidence
        mapping_eligible: true
        mapping_type: strong_inferred_applicability
  ...
  deduplication:
    patents_represent_distinct_engineering_applications: true
    independent_contributions_allowed: true
    expected_combined_raw_contribution: 40
```

The Product Match score may be further normalized by the overall company scoring
model, but neither patent may be forced back to zero solely because the patent
does not prove ONNX or existing C-code generation.

### 1.9.13 Required report behavior

Both patents SHALL appear in Relevant Evidence and SHALL support scored
Applicability Mappings.

The report SHALL visibly separate:

```text
CONFIRMED EVIDENCE
GM patent describes the neural/ML engineering solution and onboard controller context.

INFERRED NNC APPLICABILITY
Neural Net Coder is a strong implementation option for deterministic embedded
code generation, optimization/resource assessment, and verification.

QUALIFICATION STILL REQUIRED
ONNX exportability, operator compatibility, selected production model,
processor/resource constraints, and integration workflow.
```

## Version 1.10 — Neural-application-led NNC Potential Fit scoring

Version 1.10 supersedes the transitional Version 1.9 scoring override and
consolidates the decision model into the normative body of this specification.
Where a historical Version 1.1–1.9 note or regression expectation conflicts with
Sections 0, 16, 20, 21, 22, 27, or 28 as revised by Version 1.10, the Version 1.10
normative section controls.

The core business-development principle is:

```text
Target company develops or uses a neural network
with a defined engineering role
        ↓
NNC applicability already exists
        ↓
No edge deployment disclosed
        ↓
Medium / influence-stage NNC opportunity
        ↓
Embedded or onboard execution disclosed
        ↓
High NNC deployment fit
        ↓
Compatible model handover, ONNX and/or generated embedded-code workflow disclosed
        ↓
Maximum NNC potential fit
```

Neural-network-based physical-parameter estimation, virtual sensing, state
estimation, condition classification, predictive modelling, adaptive learning,
anomaly/fault detection, and perception are principal NNC signals. Physical
parameter estimation is especially strong because the trained network naturally
maps to deterministic embedded inference.

Version 1.10 introduces an authoritative cumulative **NNC Use-Case Potential Fit
Score**:

```yaml
nnc_use_case_potential_fit_score:
  company_neural_application:
    maximum_points: 55
  edge_deployment_intent:
    maximum_points: 25
  deployment_workflow_fit:
    maximum_points: 20
  total:
    maximum_points: 100
```

Within each dimension, only the strongest supported level is counted. The three
dimension scores are then summed.

This version also:

- removes edge deployment as a prerequisite for medium NNC relevance;
- keeps generic ML opportunity evidence visible without falsely creating an NNC
  mapping;
- separates current ONNX adoption from ONNX exportability/compatible model
  handover;
- separates technical fit from deployment readiness and incumbent-toolchain
  status;
- introduces `nnc_influence_opportunity`, `nnc_deployment_fit`, and
  `nnc_maximum_workflow_fit` mapping classes;
- defines company-level aggregation so multiple distinct use cases do not simply
  sum beyond 100;
- replaces conflicting Version 1.1–1.9 mapping gates, scoring rules, Relevant
  Evidence rules, and GM regression expectations in the normative sections.
...
### Target-company attribution gate

Before technical relevance is scored, the retrieved source SHALL establish a
relationship to the scanned company through a strong company anchor,
official/approved domain, validated patent assignee, approved organizational
unit, or explicit documented relationship.

Search query text, provider ranking, candidate registration, an ambiguous short
alias, or a bibliography-only name match SHALL NOT satisfy this gate.

### Neural-application gate

The primary NNC applicability signal is an explicit neural network with a
defined engineering role.

Qualifying roles include:

```yaml
qualifying_neural_roles:
  - physical_parameter_estimator
  - virtual_sensor
  - soft_sensor
  - state_estimator
  ...
  - anomaly_or_fault_detector
  - perception_model
  - neural_control_component
```

Generic `AI`, `analytics`, or `algorithm` terminology is insufficient.

A generic ML estimator without neural confirmation MAY create an
`emerging_ml_opportunity`, but SHALL NOT create a normal NNC applicability
mapping.

### Edge-deployment gate

Edge/onboard execution is **not required for NNC relevance**.

It is required to promote an influence-stage neural opportunity into a confirmed
or strongly inferred deployment-fit mapping.

Absence of edge evidence SHALL mean:

```yaml
edge_deployment_unknown:
  relevance_effect: no_edge_bonus
  exclusion_effect: none
  score_reset: false
  commercial_interpretation: dSPACE_can_influence_deployment_decision
```

### Deployment-workflow gate

ONNX, model conversion, generated C/C++, or an existing embedded-AI toolchain
are **maximum-fit/readiness signals**, not prerequisites for NNC relevance.

Their absence SHALL not zero the NNC Potential Fit score.

### Company-role gate

Silicon vendors, deployment-tool vendors, and direct code-generation competitors
SHALL not be scored like end-user engineering organizations.

### Portfolio-grounding gate

No NNC capability claim or recommendation may be published without
authoritative Neural Net Coder portfolio chunks retrieved from ChromaDB.

### Ownership gate

Neural Net Coder is a dSPACE product. The target company SHALL never be
described as owning, supplying, developing, or using it unless explicit
evidence supports that statement.

### Evidence-locality gate

The company attribution, neural role, and engineering use case used to create a
mapping SHALL belong to one coherent technical context, named project/system, or
validated patent/application family.

The pipeline SHALL NOT combine an unrelated neural paper with a generic ECU
manual merely because both belong to the same company.

Edge-deployment and deployment-workflow dimensions MAY be `unknown`; they SHALL
not be fabricated from unrelated sources.

### Mapping-promotion gate

An evidence item proving only a generic ECU, microcontroller, RAM/ROM/stack
diagnostic, or signal encoding SHALL not support an NNC mapping.

Mapping promotion SHALL follow the Version 1.10 mapping classes in Section 20.

### Relevant-Evidence promotion gate

A genuine target-company source MAY enter `Relevant Evidence` through one of
three paths:

1. `nnc_applicability_evidence` — supports a scored NNC mapping;
2. `nnc_opportunity_evidence` — commercially relevant ML activity but not enough
   evidence for an NNC mapping;
3. `contextual_company_evidence` — only when explicitly allowed for report
   context and never as NNC score support.

`general_industry_background` SHALL remain excluded from company-specific
Relevant Evidence.
...
## 9.3A NNC applicability taxonomy for neural engineering roles

The use-case taxonomy SHALL distinguish the **engineering role** of the neural
network from the physical parameter or application domain.
...
Examples of qualifying physical parameters and latent states include:

```yaml
qualifying_physical_parameters:
  - tractive_limit
  - wheel_stability
  - tire_road_friction
  - vehicle_mass
...
The list is extensible. The semantic rule is more important than the literal
term list:

> A trained neural network that infers a physical, operational, diagnostic, or
> perceptual state from measured signals is a principal NNC applicability
> signal.

The taxonomy SHALL retain a separate class for generic ML estimators when the
neural architecture is not confirmed.
...
Evidence acceptance, NNC relevance, mapping eligibility, and workflow readiness
are separate deterministic decisions.
...
- the target-company relationship is explicit or established through an approved
  attribution mechanism;
- the passage supports the normalized fact;
...
### 16.3A Mapping eligibility classes

Evidence MAY support an NNC mapping under these classes:

```yaml
mapping_classes:

  nnc_influence_opportunity:
    requirements:
      - company_attribution_established
      - neural_application_confirmed
      - defined_engineering_role
      - physical_or_embedded_adjacent_use_case
      - coherent_context
    edge_deployment_required: false
    onnx_required: false
    generated_code_required: false
  ...
  nnc_deployment_fit:
    requirements:
      ...
      - neural_application_confirmed
      - defined_engineering_role
      - embedded_or_onboard_execution_confirmed
      - coherent_context
    onnx_required: false
    generated_code_required: false

  nnc_maximum_workflow_fit:
    requirements:
      - company_attribution_established
      - neural_application_confirmed
      - edge_deployment_confirmed
      - compatible_model_handover_confirmed
      - generated_code_or_equivalent_embedded_deployment_workflow_confirmed
      - coherent_context
```

A source that confirms a neural application but does not disclose edge
deployment MAY therefore be mapping eligible as `nnc_influence_opportunity`.

### 16.3B Non-mapping opportunity evidence

Generic ML with no confirmed neural architecture MAY receive an opportunity
score, but SHALL not support an NNC applicability mapping.

Examples:

```yaml
generic_ml_estimator:
  relevance_class: nnc_opportunity_evidence
  mapping_eligible: false
  neural_application_points: 20

generic_neural_network_mention_without_defined_role:
  relevance_class: nnc_opportunity_evidence
  mapping_eligible: false
  neural_application_points: 5
```

### 16.3C Existing false-positive protections remain mandatory

- `ABS electronic control unit` in an owner's manual MAY be accepted as
  `confirmed_generic_controller_presence`, but SHALL be mapping ineligible;
...
- technical evidence not attributable to the target company SHALL contribute
  zero company score and cannot support a mapping.

### 16.3D ONNX and code-generation absence rule

```yaml
onnx_or_code_generation_missing:
  relevance_effect: no_workflow_bonus
  exclusion_effect: none
  score_reset: false
  mapping_effect:
    influence_opportunity: allowed
    deployment_fit: allowed
    maximum_workflow_fit: not_available
  interpretation: workflow_not_yet_confirmed
```

The BDA SHALL not reinterpret missing public workflow evidence as technical
incompatibility.
...
not independently support NNC mappings.

`general_industry_background` is not a target-company fact. It SHALL be stored
separately and SHALL never support company scoring or mappings.

A context bundle SHALL preserve known and unknown dimensions explicitly:
...
```json
{
  ...
  "subject_anchor": "named neural engineering application",
  "locality_mode": "section_local",
  "document_ids": ["DOC-00007"],
  "evidence_ids": ["EVD-00012", "EVD-00013"],
  "dimensions": {
    "neural_application": "confirmed",
    "engineering_role": "physical_parameter_estimator",
    "embedded_target": "unknown",
...
Company-wide aggregation MAY be used for account profiling and company-level
score aggregation, but it SHALL NOT be used to fabricate a missing neural role,
target, or workflow for a specific use case.

## 16.4 Company-specific NNC relevance classes

Version 1.10 defines four company-specific relevance classes:
...
### 16.4.1 `nnc_deployment_evidence`

Use when the source directly confirms:

- a neural application with a defined engineering role;
- embedded/onboard execution; and
- a compatible model-handover or model-to-embedded-software workflow.

This class may support `nnc_deployment_fit` or
`nnc_maximum_workflow_fit` mappings.

### 16.4.2 `nnc_applicability_evidence`

Use when the source confirms a target-company neural application with a defined
engineering role and a concrete physical/embedded-adjacent use case.

Edge deployment MAY be unknown.

```yaml
nnc_applicability_evidence_gate:
  mandatory:
    company_attribution_status: established
    neural_application_confirmed: true
    neural_network_has_defined_engineering_role: true
    concrete_engineering_application: true
...
This is the normal relevance class for an application where dSPACE can influence
the deployment architecture.

### 16.4.3 `nnc_opportunity_evidence`

Use for genuine target-company applied ML where the NNC neural fit is not yet
strong enough for a mapping.

Examples include:

- generic ML estimator without neural confirmation;
- neural network mentioned without a defined engineering role;
- ML application where company attribution is strong but technical architecture
...
nnc_opportunity_evidence:
  relevant_evidence_eligible: true
  mapping_eligible: false
  maximum_neural_application_points: 20
  generate_qualification_questions: true
```

### 16.4.4 Embedded-adjacent semantics

Terms such as `telematics unit`, `vehicle data`, `ADAS`, `sensorless`,
`real-time loading condition`, `virtual sensor`, `soft sensor`, or
`physical-system application` establish application adjacency but do not by
themselves prove embedded execution.

A confirmed neural application in such a context MAY still be
`nnc_applicability_evidence` and support `nnc_influence_opportunity`.

### 16.4.5 Cloud language

The presence of `cloud computing`, `cloud analytics`, or remote processing SHALL
not automatically reject a target-company neural application.

A clearly cloud-only implementation receives no edge-deployment points. If the
same source establishes a credible embedded/physical application for the neural
model, the neural-application score remains valid.

### 16.4.6 Claim ceiling

The evidence class SHALL never imply more than the source proves.

Examples:

```yaml
source_proves_neural_application_but_no_edge:
  allowed:
    - NNC is a credible implementation opportunity
    - dSPACE may influence the edge-deployment decision
  prohibited:
    - the company already deploys the NN on an MCU
    - the company already uses ONNX
    - the company already generates embedded C code

patent_claims_neural_embodiment:
  allowed:
    - the patent explicitly claims a neural embodiment
    - NNC applicability can be scored if the role is technically relevant
  prohibited:
    - the neural embodiment is the confirmed production implementation
```


# 17. Company Assessment JSON

The company assessment SHALL describe target-company engineering activity
independently from dSPACE product claims, while preserving enough normalized
information for deterministic NNC fit scoring.
...
```json
{
  ...
  "observed_entities": [
    {
      "target_company_entity_id": "TCE-00001",
      "name": "Battery SOH neural estimator",
      "entity_type": "neural_network_application",
      "use_case_class": "battery_health_and_state_estimation",
      "neural_role": "physical_parameter_estimator",
      "model_class": "lstm",
      "model_certainty": "confirmed",
      "target_class": "automotive_ecu",
      "edge_deployment_status": "confirmed",
  ...
  "cautious_inferences": [],
  "unknowns": [
    "Whether the neural model can be exported or converted to supported ONNX",
...
This company assessment does not itself assert that NNC is used. It is the
deterministic input to Section 20 scoring and applicability mapping.
...
## 20.2 Version 1.10 decision logic

### Gate A — Company attribution

Required for all company-specific NNC scoring and mappings.

### Gate B — Neural application

Classify the strongest supported neural-application level for each distinct
engineering use case.

```yaml
company_neural_application_scoring:
  neural_network_actually_used_for_parameter_estimation:
    points: 55

  neural_network_explicit_operational_embodiment:
    points: 50

  neural_network_explicitly_claimed_for_parameter_estimation:
    points: 45

  neural_network_optional_but_semantically_connected_to_estimator:
    points: 35

  generic_machine_learning_estimator_without_neural_confirmation:
    points: 20

  generic_neural_network_mention:
    points: 5

  no_neural_or_machine_learning_method:
    points: 0
```

The labels apply to all qualifying neural roles, not only physical-parameter
estimation. For example, a confirmed neural anomaly detector or perception model
may use the corresponding 55/50/45 evidence-strength tier.

Only the highest supported level is counted for the dimension.

### Gate C — Edge-deployment intent

Edge evidence is additive, not a prerequisite for medium NNC relevance.

```yaml
edge_deployment_scoring:
  deployed_on_mcu_ecu_soc_or_edge_ai_device:
    points: 25

  target_edge_hardware_explicitly_selected:
    points: 22

  implementation_on_onboard_controller_explicit:
    points: 18

  real_time_onboard_execution_explicit:
    points: 15

  implementable_on_ecu_or_controller:
    points: 10

  embedded_adjacent_physical_application_only:
    points: 5

  no_edge_deployment_decision_disclosed:
    points: 0
```

Only the highest supported level is counted.

### Gate D — Deployment-workflow fit

Workflow evidence raises the use case toward maximum fit.

```yaml
deployment_workflow_scoring:
  onnx_plus_generated_embedded_code:
    points: 20

  onnx_plus_explicit_edge_deployment:
    points: 17

  generated_c_or_cpp_from_neural_model:
    points: 15

  onnx_model_adoption:
    points: 12

  onnx_exportability_or_compatible_model_handover_confirmed:
    points: 10

  model_conversion_or_embedded_ai_toolchain:
    points: 8

  framework_known_and_likely_exportable:
    points: 5

  no_public_workflow_evidence:
    points: 0
```

Only the highest supported level is counted.

Current ONNX adoption and ONNX exportability are distinct facts. A company may
receive workflow-fit credit when exportability/compatible handover is confirmed
even if the current project does not yet persist ONNX artifacts.

### Gate E — Context locality

Company attribution, neural role, and engineering use case SHALL belong to one
coherent context, named project/system, or validated patent/application family.

The pipeline SHALL not construct a neural application by mixing unrelated
documents.

Edge and workflow evidence MAY come from a tightly related project source only
when the relationship to the same application is explicit.

### Gate F — Company role and portfolio grounding

Company-role controls and dSPACE portfolio grounding remain mandatory.

## 20.3 Applicability mapping classes
...
mapping_classes:
  ...
  nnc_influence_opportunity:
    requirements:
      - company_attribution_established
      - neural_application_points_at_least: 45
      - defined_neural_engineering_role
      - concrete_physical_or_embedded_adjacent_use_case
      - context_locality_pass
    edge_deployment_required: false
    onnx_required: false
    generated_code_required: false
    typical_relevance:
      - medium_nnc_relevance

  nnc_deployment_fit:
    requirements:
      - company_attribution_established
      - neural_application_points_at_least: 45
      - edge_deployment_points_at_least: 15
      - context_locality_pass
    onnx_required: false
    generated_code_required: false
    typical_relevance:
  ...
  nnc_maximum_workflow_fit:
    requirements:
      ...
      - neural_application_points_at_least: 45
      - edge_deployment_points_at_least: 15
      - deployment_workflow_points_at_least: 12
      - compatible_model_handover_confirmed
      - context_locality_pass
    typical_relevance:
      - maximum_nnc_potential_fit
```

A 35-point optional neural alternative is commercially interesting but normally
remains `nnc_opportunity_evidence` unless stronger source semantics establish a
confirmed neural role.

Generic ML without neural confirmation may score 20 as
`emerging_ml_opportunity`, but SHALL not create an NNC mapping.

### 20.3.1 Mapping evidence contract

Every published mapping SHALL persist:

```yaml
mapping_evidence_contract:
  mapping_class: required
  ...
  neural_application_points:
    minimum_for_mapping: 45
  embedded_target:
    allowed_states:
      - confirmed
      - unknown_for_influence_opportunity
  deployment_workflow:
    allowed_states:
      - confirmed
      - partial
...
The mapping text SHALL distinguish facts from inferred NNC capabilities.

For example:

```text
Confirmed company activity:
Neural tractive-limit estimator for vehicle control.

Inferred NNC applicability:
NNC could provide deterministic embedded code generation, resource assessment
and model-to-code verification.

Qualification:
Confirm exportability/ONNX operators, processor target and resource constraints.
```

## 20.4 NNC Use-Case Potential Fit scoring

### 20.4.1 Per-use-case cumulative score

For every distinct engineering application:

```text
NNC Use-Case Potential Fit =
Company Neural Application
+ Edge Deployment Intent
...
nnc_use_case_potential_fit_score:
  company_neural_application:
    maximum_points: 55
    scoring_mode: highest_single_supported_level
...
relevance_bands:
  ...
  20_to_39:
    label: emerging_ml_opportunity

  40_to_64:
    label: medium_nnc_relevance

  65_to_84:
    label: high_nnc_relevance

  85_to_100:
    label: maximum_nnc_potential_fit
```

Interpretation:

- `40–64`: neural application exists but edge deployment/workflow is not fully
  established; dSPACE can influence implementation architecture;
- `65–84`: neural application plus strong embedded/onboard deployment evidence;
- `85–100`: neural application plus strong edge deployment and compatible
  model-handover/deployment workflow.

### 20.4.3 No negative reset for unknown edge or workflow

```yaml
no_edge_deployment_evidence:
  relevance_effect: no_edge_bonus
  exclusion_effect: none
  score_reset: false
...
Explicit incompatibility is different from missing evidence and MAY reduce or
block fit after deterministic qualification.

### 20.4.4 Company-level Overall Product Match aggregation

Use-case scores SHALL not simply be summed.

```yaml
company_score_aggregation:
  base_score:
    method: highest_distinct_use_case_score

  opportunity_breadth_bonus:
    second_distinct_use_case_score_at_least_40: 10
    third_distinct_use_case_score_at_least_40: 5
    maximum_bonus: 15

  maximum_company_score: 100
```
...
```yaml
use_cases:
  predictive_battery_charging: 65
  tractive_limit_and_wheel_stability: 55

company_overall_product_match:
  base_score: 65
  breadth_bonus: 10
  total: 75
  rating: high_nnc_relevance
```

Corroborating documents for the same engineering application improve evidence
confidence but do not create a new breadth bonus.

### 20.4.5 Evidence confidence is separate
...
evidence_confidence:
  high:
    examples:
      ...
      - validated_target_company_patent
      - peer_reviewed_company_authored_publication
  medium:
    examples:
      - explicit_partner_case_study
      - professional_publication_with_verified_company_affiliation
  low:
    examples:
      - weak_third_party_summary
```

Evidence confidence SHALL be reported and used for qualification, but SHALL not
be silently mixed into the 100-point technical potential-fit formula.

### 20.4.6 Deployment readiness is separate
...
deployment_readiness_score:
  ...
  included_in_overall_product_match: false
  components:
    ONNX_export_or_compatible_handover_confirmed: 25
    supported_operator_set_confirmed: 15
    target_processor_identified: 15
    RAM_ROM_latency_constraints_known: 15
    existing_C_CPP_integration_workflow: 10
...
An unknown field contributes zero readiness points but SHALL not become a
negative applicability penalty.

### 20.4.7 Commercial toolchain status is separate

```yaml
commercial_toolchain_status:
  greenfield:
    meaning: no incumbent deployment tool identified

  manual_or_custom:
    meaning: custom or manual workflow may create strong NNC value opportunity

  incumbent_tool_identified:
    meaning: technical fit may be high but competitive displacement is required

  nnc_or_dspace_already_used:
    meaning: expansion or standardization opportunity rather than greenfield
```

A maximum technical fit SHALL not automatically be interpreted as maximum sales
whitespace.
...
```json
{
  ...
  "overall_product_match": {
    "score": 75,
    "rating": "high_nnc_relevance",
    "base_use_case_score": 65,
    "opportunity_breadth_bonus": 10
  },
  "deployment_readiness": {
    "score": 25,
    "rating": "low_or_partial"
  },
  "commercial_toolchain_status": "greenfield_or_unknown",
  "use_case_assessments": [
    {
      "use_case_assessment_id": "UCA-00001",
      "target_company_entity_id": "TCE-00001",
      "use_case_class": "battery_virtual_measurement_and_predictive_control",
      "neural_role": "physical_parameter_estimator",
      "score_components": {
        "company_neural_application": 50,
        "edge_deployment_intent": 15,
        "deployment_workflow_fit": 0
      },
      "potential_fit_score": 65,
      "relevance_band": "high_nnc_relevance",
  ...
  "mappings": [
    {
      ...
      "mapping_class": "nnc_deployment_fit",
      "target_company_entity_id": "TCE-00001",
      "target_company_activity": "Neural battery-state estimator for onboard charging control",
      "neutral_engineering_need_id": "deterministic_embedded_neural_inference",
      "dspace_capability_id": "deterministic_c_cpp_generation",
      "dspace_portfolio_item_id": "neural_net_coder",
      "target_company_evidence_ids": ["EVD-00001"],
      "supporting_dspace_document_chunk_ids": ["NNC-DOC-001-P003-C002"],
      "confirmed_facts": [
        "A neural estimator is described for the target-company battery application",
        "Onboard controller context is explicit"
      ],
      "inferred_nnc_applicability": [
        "NNC could support deterministic embedded code generation and verification"
      ],
      "assumptions": [
        "The trained neural component can be exported or converted to a supported ONNX graph"
      ],
      "qualification_questions": [
        "Which operators and precision are used?",
        "Which processor, compiler, RAM/ROM and latency constraints apply?"
      ]
    }
  ],
  "score_trace": [],
...
Every mapping SHALL contain the complete scope-safe chain and its mapping class.
The commercial name SHALL come from the validated portfolio profile.


# 22. Final Assessment JSON

The final assessment SHALL combine company facts, use-case fit, company-level
aggregation, portfolio grounding, deployment readiness, narrative status, and
report status without allowing the LLM to regenerate deterministic values.
...
portfolio-grounding failure, degraded search, and scoring incompleteness
consistently with the run manifest.
...
It SHALL show, in concise form:

- Overall Product Match score and relevance band;
- strongest NNC use case;
- additional distinct strong opportunities;
...
```text
Overall Product Match: 75/100 — High NNC Relevance
Deployment Readiness: Partial / Unknown

GM has two distinct neural engineering opportunities: predictive battery
charging and tractive-limit/wheel-stability estimation. The applications show
strong NNC potential even though public evidence does not establish an existing
ONNX/code-generation workflow.
```
...
|---|---|
| Neural Net Coder | `<target-company activity> → <NNC implementation opportunity> → <verified NNC capability> — <mapping class / qualification>` |

The mapping text MAY include a short qualifier such as:

- `Influence opportunity — edge target not yet public`;
- `Deployment fit — onboard execution confirmed`;
- `Maximum workflow fit — compatible handover/code-generation workflow confirmed`.
...
Every published source SHALL be genuine target-company evidence. General
industry background remains excluded.

### Path A — `nnc_applicability_evidence`

Requirements:

- company attribution established;
- accepted target-company evidence;
- neural network has a defined engineering role;
- concrete engineering use case;
- passed context-locality rules;
- supports a published NNC mapping.

Edge deployment and ONNX/code-generation workflow MAY be unknown for
`nnc_influence_opportunity`.

### Path B — `nnc_opportunity_evidence`

Requirements:

- company attribution established;
- accepted target-company evidence;
- concrete ML activity;
- neural applicability not yet strong enough for a mapping;
- explicit qualification/uncertainty statement.

This path may carry an emerging opportunity score such as 20 for a generic ML
estimator, but SHALL not support an NNC mapping.
...
relevant_evidence_promotion:
  ...
  path_a_nnc_applicability:
    require_relevance_class: nnc_applicability_evidence
    require_mapping_eligible: true
    require_published_mapping_reference: true
    require_neural_role: true
    edge_deployment_may_be_unknown_for_influence_mapping: true
    workflow_may_be_unknown: true

  path_b_nnc_opportunity:
    require_relevance_class: nnc_opportunity_evidence
    require_mapping_eligible: false
    require_published_mapping_reference: false
    require_uncertainty_disclosure: true
```

The builder SHALL evaluate each source/evidence item independently.

Examples that SHALL still be omitted:

- an unrelated TinyML paper returned by a company-alias query;
- `G.M.` personal initials in a bibliography;
- an owner's manual that only mentions an ABS ECU;
- OBD RAM/ROM/stack/watchdog diagnostics;
- diagnostic `linear quantization`;
- sources that merely prove the company manufactures vehicles with ECUs.

The visible table remains:

| No. | Evidence | URL |
|---:|---|---|
| 1 | `<concise company-specific evidence>` | `<source URL>` |

For `nnc_applicability_evidence`, the description SHOULD explicitly distinguish
confirmed facts from the NNC inference when needed.
...
- company-attribution gate;
- neural-role classification;
- generic-AI rejection;
- generic-ML opportunity scoring without mapping;
- non-neural-model classification;
- SoC acronym disambiguation;
- MCU/ECU target normalization;
- edge-deployment score levels;
- cloud/GPU handling;
- company-role classification;
- direct-competitor handling;
- applicability mapping-class selection;
- per-use-case 55/25/20 scoring;
- mutually exclusive highest-level scoring inside each dimension;
- relevance-band boundaries;
- missing-edge no-reset behavior;
- missing-ONNX/code-generation no-reset behavior;
- ONNX adoption vs ONNX exportability distinction;
- company-level best-use-case plus breadth-bonus aggregation;
- duplicate/corroborating-source suppression;
- deployment-readiness score separation;
- incumbent/commercial-toolchain status separation;
- NNC portfolio ingestion and grounding;
- ownership and attribution validation;
- report order and two-column mapping table;
- visible evidence shortening without JSON mutation;
- workbook update eligibility;
- required-query execution and degraded-run behavior;
- context-bundle construction and locality validation;
- rejection of company-wide cross-document gate mixing;
- source-type blocking for owner/OBD/service manuals;
- quantization semantic disambiguation;
- patent-native assignee enumeration and provider quality;
- complete patent term coverage and pagination;
- two-path Relevant Evidence promotion:
  `nnc_applicability_evidence` or `nnc_opportunity_evidence`;
- General Motors dual-patent discovery;
- General Motors dual-patent Version 1.10 scoring.
...
1. a confirmed neural application with no disclosed edge deployment receives
   medium NNC relevance rather than zero relevance;
2. a confirmed neural application with onboard/edge deployment receives high
   NNC relevance;
3. a confirmed neural application with strong compatible model-handover and
   embedded code-generation workflow can reach maximum NNC potential fit;
4. a generic ML estimator without neural confirmation may receive an emerging
   opportunity score but cannot create an NNC mapping;
5. a generic neural-network mention without a defined engineering role receives
   only minimal opportunity credit;
6. a linear-regression-only application does not receive a normal neural NNC
   mapping;
7. missing ONNX evidence contributes zero workflow bonus but does not reset the
   use-case score;
8. missing edge evidence contributes zero edge bonus but does not reset the
   neural-application score;
9. ONNX exportability and current ONNX adoption are represented separately;
10. per-dimension scores use the strongest supported level only;
11. two documents supporting the same application do not create two independent
    use-case scores;
12. two distinct engineering applications may contribute an opportunity-breadth
    bonus;
13. company score aggregation never exceeds 100;
14. deployment readiness is reported separately from Overall Product Match;
15. incumbent-toolchain status does not change technical-fit evidence into a
    false low score;
16. an owner's manual passage containing only an ABS ECU remains mapping
    ineligible;
17. neural evidence from one project cannot be combined with generic ECU
    evidence from another project;
18. `linear quantization` remains diagnostic signal encoding unless explicit
    model context proves otherwise;
19. company attribution failure blocks all company scoring;
20. portfolio grounding failure blocks publication;
21. `US12246700B2` is discovered without a production seed and scores:
    neural application 45 + edge 10 + workflow 0 = 55,
    `medium_nnc_relevance`, `nnc_influence_opportunity`;
22. `US20240302440A1` is discovered without a production seed and scores,
    conservatively by default:
    neural application 50 + edge 15 + workflow 0 = 65,
    `high_nnc_relevance`, `nnc_deployment_fit`;
23. the two GM patents are treated as distinct engineering applications;
24. the GM company aggregation from only these two use cases produces:
    base 65 + breadth bonus 10 = Overall Product Match 75;
25. neither GM patent is described as proof that GM currently uses ONNX, NNC, or
    generated C code.
...
This fixture verifies discovery, organizational-unit attribution, and Version 1.10
emerging-ML opportunity scoring without falsely asserting a neural architecture.
...
false_negative_regression_fixture:
  ...
  expected:
    ...
    score:
      company_neural_application: 20
      edge_deployment_intent: 0
      deployment_workflow_fit: 0
      nnc_use_case_potential_fit_score: 20
    relevance_band: emerging_ml_opportunity
    mapping_eligible: false
    applicability_mapping_created: false
    relevant_evidence: true
...
false_negative_regression_fixture:
  ...
  expected:
    ...
    score:
      ...
      edge_deployment_intent: 0
      deployment_workflow_fit: 0
      nnc_use_case_potential_fit_score: 50
    relevance_band: medium_nnc_relevance
    relevance_class: nnc_applicability_evidence
    relevant_evidence: true
    mapping_eligible: true
    mapping_class: nnc_influence_opportunity
...
false_negative_regression_fixture:
  ...
  expected_classification:
    ...
    score:
      company_neural_application:
        classification: neural_network_explicitly_claimed_for_parameter_estimation
        points: 45
      edge_deployment_intent:
        classification: implementable_on_ecu_or_controller
        points: 10
      deployment_workflow_fit:
        classification: no_public_workflow_evidence
        points: 0
      nnc_use_case_potential_fit_score: 55
    relevance_band: medium_nnc_relevance
    relevance_class: nnc_applicability_evidence
    relevant_evidence: true
    mapping_eligible: true
  ...
  expected_report:
    ...
    applicability_mapping_required: true
    preserve_production_algorithm_uncertainty: true
```
...
false_negative_regression_fixture:
  ...
  expected_classification:
    ...
    score:
      company_neural_application:
        classification: neural_network_explicit_operational_embodiment
        points: 50
      edge_deployment_intent:
        classification: real_time_onboard_execution_explicit
        points: 15
      deployment_workflow_fit:
        classification: no_public_workflow_evidence
        points: 0
      nnc_use_case_potential_fit_score: 65
    relevance_band: high_nnc_relevance
    relevance_class: nnc_applicability_evidence
    relevant_evidence: true
    mapping_eligible: true
...
patent_native_regression_fixture:
  ...
  expected_discovered_patents:
    - publication_number: US12246700B2
      ...
      expected_relevance_class: nnc_applicability_evidence
      expected_relevant_evidence: true
      expected_mapping_eligible: true
      expected_mapping_class: nnc_influence_opportunity
      expected_use_case_score: 55
    ...
    - publication_number: US20240302440A1
      ...
      expected_relevant_evidence: true
      expected_mapping_eligible: true
      expected_mapping_class: nnc_deployment_fit
      expected_use_case_score: 65
...
## Appendix G.10 Version 1.10 GM scoring regression

```yaml
fixture_id: GM-NNC-V1_10-USE-CASE-FIT
target_company: General Motors

discovery:
  follow_version_1_8_patent_native_process: true
  production_seed_urls_allowed: false
  publication_numbers_as_production_queries_allowed: false

use_cases:

  tractive_limit_and_wheel_stability:
    source_assertion: US12246700B2
    score:
      company_neural_application:
        classification: neural_network_explicitly_claimed_for_parameter_estimation
        points: 45
  ...
  predictive_battery_charging_and_virtual_measurements:
    source_assertion: US20240302440A1
    score:
      company_neural_application:
        classification: neural_network_explicit_operational_embodiment
        points: 50
...
company_aggregation:
  highest_distinct_use_case_score: 65
  second_distinct_use_case_score_at_least_40_bonus: 10
  opportunity_breadth_bonus: 10
  overall_product_match: 75
  overall_rating: high_nnc_relevance

deployment_readiness:
  ONNX_export_or_compatible_handover: unconfirmed
  generated_code_workflow: unconfirmed
  effect_on_product_match: none
  qualification_required: true

prohibited_claims:
  - GM already uses Neural Net Coder
  - GM already uses ONNX for these inventions
  - GM already generates C code from these models
  - the optional neural embodiment in US12246700B2 is the confirmed production implementation
```
...
|---|---|---|
| 1.10.0 | 2026-08-05 | Replaced transitional v1.9 mapping-point scoring with cumulative 55/25/20 NNC Use-Case Potential Fit; made neural engineering role the principal relevance signal; removed edge/ONNX/code-generation as relevance reset gates; added influence/deployment/maximum mapping classes, ONNX exportability distinction, company best-use-case plus breadth-bonus aggregation, separate readiness/confidence/toolchain status, revised Relevant Evidence promotion, and authoritative GM 55/65 use-case plus 75 company-score regression |
| 1.9.0 | 2026-08-05 | Separated technical NNC applicability from current ONNX/code-generation adoption; added `nnc_applicability_evidence`, `strong_inferred_applicability`, high score contributions, separate deployment-readiness scoring, ONNX/operator qualification instead of hard gating, use-case caps, and a dual-GM regression requiring scored mappings for US12246700B2 and US20240302440A1 |
