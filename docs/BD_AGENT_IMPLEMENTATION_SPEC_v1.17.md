...
43. Version 1.17 SHALL not modify deterministic applicability mapping rules,
...
51. Explicit seed registration SHALL be completed, validated, and persisted
    before candidate-budget filtering.
52. `seeds_registered` SHALL be derived exclusively from the immutable
    pre-budget seed-registration registry.
53. Explicit valid seed URLs SHALL receive protected retrieval priority and
    SHALL not be removed by host caps, total candidate caps, third-party caps,
    or deterministic domain/type ranking.
54. Candidate budgets SHALL apply only after protected seed slots have been
    reserved.
55. Seed registration, retrieval selection, retrieval attempt, retrieval result,
    and evidence acceptance SHALL be represented as separate states and metrics.
56. A seed that is registered but later fails retrieval SHALL not retroactively
    change `seeds_registered` to false.
57. The generic status
    `incomplete_search_mandatory_provider_not_executed` SHALL not be used for a
    seed-registration defect. Seed-specific failure codes SHALL be used.
58. Version 1.17 SHALL not modify taxonomy matching, evidence acceptance,
    applicability mappings, dSPACE grounding, deterministic scoring, or
    workbook-update rules.
59. Create tests for every mandatory requirement in this specification.
60. Record implementation deviations in `docs/IMPLEMENTATION_NOTES.md`.
...
The `ddgs` package is mandatory because Version 1.17 requires real execution of
text-search queries.
...
│               ├── seed_registration.json
│               ├── candidate_budget_audit.json
...
└── tests/
    ...
    │   ├── test_seed_registration_pre_budget.py
    │   ├── test_seed_protected_priority.py
    │   ├── test_seed_registration_vs_retrieval.py
    │   ├── test_seed_specific_status_codes.py
    │   ├── test_siemens_734_candidate_seed_regression.py
...
<COMPANY_SLUG>_<RUN_TIMESTAMP>_seed_registration.json
<COMPANY_SLUG>_<RUN_TIMESTAMP>_candidate_budget_audit.json
...
minimum web-evidence-count gate is introduced in Version 1.17.
...
Version 1.17 SHALL use the following provider and discovery combination by
default:
...
No provider outside this set SHALL be enabled by default in Version 1.17.
...
Version 1.17 broadens collection but does not require a minimum accepted
...
- thirteen valid Siemens seed URLs remain `seeds_registered: true` when the
  combined canonical candidate set contains 734 URLs;
- all thirteen valid registered seeds are protected from host and total
  candidate caps;
- the non-seed portion is reduced independently to its configured budget;
- a registered seed with retrieval failure leaves `seeds_registered: true`;
- a duplicate configured seed mapping to an existing canonical URL does not
  cause registration failure;
- an invalid seed URL produces a seed-specific configuration failure;
- the generic mandatory-provider status is not used for seed-registration
  defects;
- Siemens Energy continues into evidence processing, applicability assessment,
  PDF generation and workbook update when registration is complete.
...
### 29.13A Explicit Seed Registration and Protected Retrieval

Explicit seed URLs are authoritative discovery inputs supplied by the
company-source profile. They are not search-provider results and SHALL not be
treated as ordinary ranked candidates.

The processing order SHALL be:

```text
Load explicit seed URLs
        ↓
Validate syntax and safety policy
        ↓
Canonicalize and map configured entries to canonical source records
        ↓
Register all valid explicit seeds in the immutable pre-budget registry
        ↓
Persist seed-registration compliance
        ↓
Union seeds with search, sitemap, crawler and PDF-discovery candidates
        ↓
Canonicalize and deduplicate the full candidate set
        ↓
Reserve protected retrieval slots for registered seeds
        ↓
Rank and budget only the remaining non-protected candidates
        ↓
Retrieve protected seeds and selected ordinary candidates
```

Budget filtering SHALL never be used to calculate registration compliance.

#### 29.13A.1 Seed lifecycle states

Each configured seed entry SHALL progress through distinct states:

```text
configured
validated | rejected_invalid | rejected_safety_policy
canonicalized
registered
selected_for_retrieval
retrieval_attempted
retrieved | retrieval_failed | retrieval_skipped_safety
parsed | parse_failed
accepted_as_evidence | rejected_as_evidence | discovery_only
```

These states SHALL not be collapsed into one Boolean.

#### 29.13A.2 Immutable pre-budget registry

`seed_registration_service.py` SHALL persist:

```text
seed_registration.json
```

before candidate-budget filtering begins.
...
```json
{
  ...
  "configured_seed_entry_count": 13,
  "valid_seed_entry_count": 13,
  "unique_canonical_seed_count": 13,
  "registered_seed_count": 13,
  "invalid_seed_count": 0,
  "safety_rejected_seed_count": 0,
  "seed_registration_status": "complete",
  "seeds_registered": true,
  "records": [
    {
      "configured_seed_index": 1,
      "configured_url": "https://example.com/publications",
      "canonical_url": "https://example.com/publications",
      "registration_status": "registered",
      "candidate_source_id": "SRC-000001",
      "protected_for_retrieval": true
    }
  ]
}
```

`seed_registration.json` SHALL be immutable after the registration phase.
Retrieval outcomes SHALL be stored in later artifacts rather than rewriting
registration history.

#### 29.13A.3 Registration compliance

`seeds_registered` SHALL be calculated only as:

```python
seeds_registered = (
    configured_seed_entry_count > 0
    and invalid_seed_count == 0
    and safety_rejected_seed_count == 0
    and registered_seed_entry_count == valid_seed_entry_count
)
```

When several configured entries canonicalize to the same URL, each configured
entry SHALL retain a mapping to the shared canonical candidate source. Such
deduplication SHALL not cause registration failure.
...
not_configured
complete
partial_invalid_configuration
partial_safety_rejection
failed_registration
```

#### 29.13A.4 Protected seed retrieval priority

Every valid registered seed SHALL be marked:

```text
protected_for_retrieval: true
```

Protected seeds SHALL be selected before ordinary candidates.

The following ordinary-candidate controls SHALL not remove protected seeds:

- maximum candidates per host;
- maximum canonical candidates;
- maximum low-priority third-party pages;
- career-page limits;
- domain/type ranking;
- provider-result priority;
- ordinary candidate relevance thresholds.

Safety, protocol, URL-validity, malware, robots-policy, legal, and explicit
domain-deny rules MAY prevent retrieval. Such rejection SHALL use a specific
seed status and SHALL remain visible.

#### 29.13A.5 Budget reservation

Recommended configuration:

```yaml
seed_policy:
  explicit_seeds_mandatory: true
  protect_valid_seeds_for_retrieval: true
  maximum_explicit_seed_entries_per_company: 100
  fail_if_seed_limit_exceeded: true
  exempt_protected_seeds_from_host_caps: true
  exempt_protected_seeds_from_general_candidate_cap: true

candidate_budget:
  maximum_canonical_non_seed_candidates_per_company: 500
  maximum_retrieved_non_seed_documents_per_company: 250
```

Protected seeds SHALL use a separate reserved segment. The effective retrieval
set is:

```text
all protected valid seeds
+
budgeted non-seed candidates
```

The implementation SHALL report both counts separately.

#### 29.13A.6 Registration versus retrieval

A registered seed may later fail retrieval:
...
Such a failure MAY create:

```text
quality_status: degraded
degradation_flag: explicit_seed_retrieval_partial
```

It SHALL not retroactively create:

```text
seeds_registered: false
```

#### 29.13A.7 Seed-specific failure statuses

The generic provider status:

```text
incomplete_search_mandatory_provider_not_executed
```

SHALL NOT represent seed-registration failures.

Use:

```text
failed_configuration_invalid_seed_url
failed_configuration_seed_limit_exceeded
incomplete_search_seed_registration_failed
incomplete_search_seed_safety_rejection
```

A provider execution status is appropriate only for actual search providers.

#### 29.13A.8 Mandatory gate timing

The mandatory seed-registration gate SHALL execute immediately after
`seed_registration.json` is persisted and before ordinary candidate ranking.

The gate SHALL not be re-evaluated from the post-budget retrieval selection.

#### 29.13A.9 Required metrics

The run manifest and source-discovery report SHALL include:

```text
configured_seed_entry_count
valid_seed_entry_count
unique_canonical_seed_count
registered_seed_entry_count
protected_seed_count
protected_seeds_selected_for_retrieval
protected_seed_retrieval_attempted_count
protected_seed_retrieval_success_count
protected_seed_retrieval_failed_count
ordinary_candidates_before_budget
ordinary_candidates_selected_after_budget
ordinary_candidates_dropped_by_budget
```

### 29.13B Candidate URL Budget Enforcement
...
candidate_budget:
  maximum_canonical_non_seed_candidates_per_company: 500
  maximum_retrieved_non_seed_documents_per_company: 250
  maximum_non_seed_candidates_per_host: 120
  maximum_career_pages_per_company: 60
  maximum_low_priority_third_party_pages: 40
  persist_dropped_candidate_summary: true
```

When the canonical non-seed candidate set exceeds the cap, selection SHALL prioritize:
...
protected_seed_urls_registered
protected_seed_urls_selected_for_retrieval
non_seed_candidate_urls_before_budget
non_seed_candidate_urls_selected_for_retrieval
non_seed_candidate_urls_dropped_by_budget
...
The 500-candidate hard cap applies to canonical non-seed candidates. Protected
explicit seeds are reported separately and do not consume or compete for those
slots.

`candidate_budget_audit.json` SHALL prove that seed registration was completed
before budget application.
...
`incomplete_search_mandatory_provider_not_executed` SHALL be reserved for an
actual mandatory search provider that was not executed. Explicit seeds are
configuration inputs and SHALL use seed-specific statuses.

---

### REQ-SEED-ART-001 — Seed registration artifact

`seed_registration.json` SHALL be written before candidate budgeting and SHALL
contain only registration-phase facts.

### REQ-SEED-ART-002 — Candidate budget audit

`candidate_budget_audit.json` SHALL contain:

```json
{
  "protected_seed_count": 13,
  "protected_seed_selected_count": 13,
  "non_seed_candidates_before_budget": 721,
  "non_seed_candidates_selected_after_budget": 286,
  "non_seed_candidates_dropped_by_budget": 435,
  "seed_registration_source": "seed_registration.json",
  "seed_registration_status_used_for_gate": "complete"
}
```

### REQ-SEED-ART-003 — Cross-artifact consistency
...
Post-budget artifacts MAY report seed retrieval selection and outcome, but SHALL
not rewrite the pre-budget `seeds_registered` value.
...
degradation_flags:
  ...
  - explicit_seed_retrieval_partial
  - explicit_seed_parse_partial
```
...
After successful structural validation, Python SHALL apply the Version 1.17
...
There is no default “skip completed companies” policy in Version 1.17.
...
`--reprocess-existing-ratings` SHALL NOT be used in Version 1.17.
...
selected `Y` row is the default Version 1.17 behavior.
...
## 40. Version 1.17 Runtime, Model, Domain, and Regression Controls

### 40.1 Authoritative Production Models

Version 1.17 defines the following production models:
...
The active Version 1.17 models remain:
...
## 42. Version 1.17 Directory, Workbook, and Cycle-Selection Contract

### 42.1 Authoritative Changes

Version 1.17 makes these three changes authoritative:
...
`Companies.xlsx`; legacy binary `.xls` format is not part of Version 1.17.

---

## 43. Version 1.17 Model Assignment Contract

### 43.1 Authoritative Model Assignments

Version 1.17 fixes the production model assignments as follows:
...
`qwen3:8b` is not part of the Version 1.17 production design.
...
### 44.1 Root Cause Addressed by Version 1.17
...
Version 1.17 treats this condition as an incomplete implementation, not a
...
OCR is not mandatory for Version 1.17, but silent ingestion of an empty
...
### 45.1 Root Cause Addressed by Version 1.17
...
Version 1.17 defines this as an incomplete search implementation.
...
Version 1.17 SHALL not introduce a minimum accepted-evidence-count gate.
...
## 46. Version 1.17 Cost-Free Search Configuration and Quota Enforcement
...
The following are conservative software caps for the Version 1.17
configuration:
...
CODER has completed the Version 1.17 search configuration only when:
...
## 47. Version 1.17 Gemma Runtime and Assessment-Report Contract
...
Version 1.17 addresses the narrative path without changing the authority of the
deterministic assessment.
...
CODER has completed Version 1.17 only when:
...
## 48. Version 1.17 Provider Credentials and PDF Presentation Contract
...
## 49. Version 1.17 dSPACE Product-Ownership and Narrative-Safety Contract
...
Regression tests SHALL compare representative Version 1.16 and Version 1.17
...
## 50. Version 1.17 Degraded-Run Reliability and Observability Contract
...
Version 1.17 SHALL implement corrections in this order:
...
## 51. Version 1.17 Seed Registration and Candidate-Budget Isolation Contract

### 51.1 Defect Addressed

The Siemens Energy run registered 13 explicit seed URLs and later discovered
734 unique canonical candidates.

Candidate-budget filtering retained 299 candidates. The implementation then
incorrectly recalculated seed-registration compliance from the filtered list.
Because some explicit seeds had been removed by host, source-class or total
limits, the run produced:

```json
{
  "seed_urls_attempted": 13,
  "seeds_registered": false
}
```
...
This was a software defect. Registration compliance and retrieval-budget
selection are different concerns.

### 51.2 Corrected Ordering

The authoritative sequence is:

```text
Register and validate all explicit seeds
        ↓
Persist immutable registration compliance
        ↓
Discover, canonicalize and deduplicate all candidates
        ↓
Reserve all valid registered seeds
        ↓
Apply budgets to remaining non-seed candidates
        ↓
Retrieve protected seeds and selected non-seeds
```

### 51.3 Correct Meaning of `seeds_registered`

`seeds_registered` answers:

> Were all valid explicit seeds accepted and registered before budgeting?

It SHALL NOT answer:

> Did every seed survive later retrieval-selection budgets?

### 51.4 Protected Priority

Valid explicit seeds SHALL survive:

- maximum candidates per host;
- maximum non-seed candidates;
- low-priority third-party limits;
- deterministic source ranking;
- candidate-type limits.

Only URL validity, safety, legal policy, robots policy, malware protection, and
explicit deny rules may block them.

### 51.5 Separate Metrics

Required metrics:

| Phase | Metric |
|---|---|
| Configuration | `configured_seed_entry_count` |
| Validation | `valid_seed_entry_count` |
| Registration | `registered_seed_entry_count` |
| Canonicalization | `unique_canonical_seed_count` |
| Selection | `protected_seed_selected_count` |
| Retrieval | `protected_seed_retrieval_attempted_count` |
| Retrieval | `protected_seed_retrieval_success_count` |
| Retrieval | `protected_seed_retrieval_failed_count` |
| Evidence | accepted/rejected/discovery-only seed-derived evidence |

### 51.6 Correct Status Behavior

When all valid seeds register:
...
When registration actually fails:

```text
incomplete_search_seed_registration_failed
```

The generic mandatory-provider status SHALL not be used.

### 51.7 Siemens Regression Example

Input:

```text
explicit configured seeds: 13
unique canonical candidates: 734
```

Expected selection:

```text
protected seeds selected: 13
non-seed candidates before budget: 721
non-seed candidates selected: configured non-seed budget
```
...
```text
evidence extraction
→ bge-large processing
→ dSPACE applicability assessment
→ Gemma or deterministic fallback
→ PDF generation
→ Companies.xlsx update
```

### 51.8 Normative Requirements

- **REQ-SEED-101:** Seed registration SHALL precede candidate budgeting.
- **REQ-SEED-102:** The pre-budget seed registry SHALL be immutable.
- **REQ-SEED-103:** `seeds_registered` SHALL be calculated from the pre-budget
  registry.
- **REQ-SEED-104:** Valid registered seeds SHALL be protected for retrieval.
- **REQ-SEED-105:** Ordinary candidate budgets SHALL not remove protected
  seeds.
- **REQ-SEED-106:** Host caps SHALL apply only to non-seed candidates.
- **REQ-SEED-107:** Seed registration and retrieval SHALL have separate
  statuses.
- **REQ-SEED-108:** Retrieval failure SHALL not change registration compliance.
- **REQ-SEED-109:** Seed-registration failures SHALL use seed-specific status
  codes.
- **REQ-SEED-110:** Duplicate configured entries SHALL map to canonical records
  without false failure.
- **REQ-SEED-111:** The Siemens 13-seed/734-candidate regression case SHALL
  continue beyond discovery.
- **REQ-SEED-112:** Deterministic assessment and scoring rules SHALL remain
  unchanged.

### 51.9 Definition of Complete Implementation

CODER has completed Version 1.17 only when:

- seed registration is persisted before budget execution;
- the registration artifact cannot be mutated by later retrieval stages;
- every valid registered seed receives protected selection;
- non-seed budgets are calculated independently;
- cross-artifact seed metrics are consistent;
- seed retrieval failures create degradation, not false registration failure;
- the generic mandatory-provider status is not emitted for seed defects;
- the Siemens regression fixture completes assessment and reporting;
- deterministic regression outputs remain unchanged;
- all unit, integration and end-to-end tests pass.
