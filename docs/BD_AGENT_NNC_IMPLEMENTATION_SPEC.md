...
**Design document version:** `1.2.0`  
**Runtime compatibility tag:** `1.2`  
**Date:** `2026-08-03`
...
- **Mapping-promotion gate:** an evidence item that proves only a generic ECU,
  ...
  not support an NNC mapping and SHALL not enter `Relevant Evidence`.
...
At least one enabled provider SHALL execute every mandatory query.
...
Candidate ranking SHALL prioritize:

1. protected explicit seeds;
2. official company technical pages and PDFs;
3. official research, product, engineering, career, and publication pages;
4. named partner or university sources with direct evidence;
5. primary papers and proceedings;
6. credible third-party technical sources;
7. generic summaries and marketing aggregators.

A candidate URL limit SHALL be applied only after additive discovery,
canonicalization, deduplication, and protected-seed reservation.
...
promote a source to `Relevant Evidence`.
...
applicability mapping, score contribution, or Relevant Evidence entry.
...
The section SHALL list only distinct target-company web pages or PDFs that have
at least one evidence item satisfying all of the following:

- `company_attribution.status == established`;
- `attributed_company_id == scan_target_company_id`;
- `assessment.status == accepted`;
- `mapping_eligibility.eligible == true`;
- the evidence belongs to a passed context bundle;
- the evidence ID is used by a published applicability mapping;
- the source-type promotion policy permits publication.
...
- Relevant Evidence promotion based only on mapping-eligible evidence.
...
12. create NNC applicability rules and score trace;
13. prepare and approve the NNC portfolio document index;
14. ingest and validate the NNC ChromaDB collection;
15. adapt query templates and provider tests;
16. adapt prompt ownership and substantive validation;
17. adapt report wording while retaining Version 1.26 layout;
18. run unit and integration tests;
19. run controlled example companies;
20. compare artifacts against this specification;
21. document every deviation.
