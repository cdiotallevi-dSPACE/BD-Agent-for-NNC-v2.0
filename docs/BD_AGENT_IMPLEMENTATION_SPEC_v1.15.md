...
43. Version 1.15 SHALL not modify deterministic applicability mapping rules,
    mapping scores, or dSPACE portfolio selection logic.
44. Create tests for every mandatory requirement in this specification.
45. Record implementation deviations in `docs/IMPLEMENTATION_NOTES.md`.
...
The `ddgs` package is mandatory because Version 1.15 requires real execution of
text-search queries.
...
│                   └── errors.log
├── prompts/
│   ├── company_summary_system_v2.txt
│   ├── company_summary_user_template_v2.txt
│   ├── company_summary_output_schema_v2.json
│   ├── company_summary_prompt_manifest_v2.yaml
...
Open data/Companies.xlsx from the configured application root
  ...
  -> for each selected company:
       ...
       -> validate LLM JSON and product ownership, or use the expanded
...
minimum web-evidence-count gate is introduced in Version 1.15.
...
Version 1.15 SHALL use the following provider and discovery combination by
default:
...
No provider outside this set SHALL be enabled by default in Version 1.15.
...
Version 1.15 broadens collection but does not require a minimum accepted
...
```yaml
prompt_bundle:
  bundle_id: company_summary_v2.0
  system_prompt:
    path: prompts/company_summary_system_v2.txt
    version: "2.0.0"
    sha256: "..."
  user_template:
    path: prompts/company_summary_user_template_v2.txt
    version: "2.0.0"
    sha256: "..."
  output_schema:
    path: prompts/company_summary_output_schema_v2.json
    version: "2.0.0"
    sha256: "..."
  ownership_contract:
...
### 38.13 Python Output and Product-Ownership Validation

Python SHALL validate JSON structure and narrative semantics before publication.
...
- a dSPACE product or service is assigned to the target company;
- the first narrative mention of a recommended dSPACE offering lacks explicit
  dSPACE attribution;
- the returned owner or supplier differs from `dSPACE GmbH`.

`narrative_ownership_validator.py` SHALL receive:

```text
target_company_name
target_company_aliases
dspace_portfolio_item_ids
dspace_commercial_names
dspace_product_aliases
narrative_fields
```

The validator SHALL inspect every narrative field, including:

- executive summary;
- technology and engineering context;
- identified-need explanations;
- portfolio recommendation assessments;
- closing assessment;
- recommended action text where product names are mentioned.

#### Prohibited ownership patterns

For every dSPACE product alias, Python SHALL detect at least:

```text
<TARGET COMPANY>'s <DSPACE PRODUCT>
<TARGET COMPANY>’s <DSPACE PRODUCT>
their <DSPACE PRODUCT>
<TARGET COMPANY> owns <DSPACE PRODUCT>
<TARGET COMPANY> offers <DSPACE PRODUCT>
<TARGET COMPANY> provides <DSPACE PRODUCT>
<TARGET COMPANY> supplies <DSPACE PRODUCT>
<TARGET COMPANY> sells <DSPACE PRODUCT>
```

The implementation SHOULD use sentence parsing and normalized alias matching in
addition to regular expressions to reduce false negatives.

A possessive reference to the target company's engineering needs, projects,
technologies, products, systems or activities SHALL remain permitted.

#### Required first-mention attribution

The first narrative mention of every recommended dSPACE offering SHALL identify dSPACE.

The first mention of each recommended dSPACE product or service SHALL include
one of these attribution forms within the same sentence or configured
attribution window:
...
<PRODUCT> from dSPACE
dSPACE <PRODUCT>
<PRODUCT>, a dSPACE product
<PRODUCT>, a dSPACE service
```

Recommended attribution window:

```yaml
narrative_ownership_validation:
  first_mention_attribution_window_characters: 100
  require_attribution_for_each_recommended_item: true
```
...
```json
{
  ...
  "violations": [
    {
      "rule_id": "target_company_possessive_dspace_product",
      "dspace_portfolio_item_id": "electrical_power_systems_simulation_package_epss",
      "product_alias": "power systems simulation package",
      "matched_text": "Schneider Electric's power systems simulation package",
...
1. classify the Local LLM result as `invalid_response`;
2. set `failure_code` to `llm_product_ownership_violation` or
   `llm_missing_dspace_product_attribution`;
3. do not retry Gemma;
4. use the deterministic fallback narrative;
5. preserve the deterministic score;
6. preserve evidence, applicability mappings and grounding;
7. preserve the score already written or pending for `Companies.xlsx`;
8. log the violation without exposing private model reasoning.

Prompt instructions alone SHALL NOT be treated as sufficient validation.

### REQ-OWNERSHIP-004 — Deterministic narrative validation

Python SHALL validate product ownership and role attribution after generation.

### REQ-OWNERSHIP-005 — Ownership violation handling

An ownership violation SHALL invalidate only the LLM narrative and SHALL
trigger the deterministic fallback.

### REQ-OWNERSHIP-006 — No automatic ownership rewrite

Version 1.15 SHALL not silently rewrite an invalid Gemma sentence into an
accepted narrative. The invalid response SHALL be rejected so that the
deterministic fallback remains auditable.


### REQ-OWNERSHIP-007 — Applicability rules unchanged

The ownership correction SHALL not modify deterministic applicability mappings,
hard-rule outcomes, portfolio grounding, or scores.
...
llm:
  company_summary:
    ...
    prompt_bundle_id: company_summary_v2.0
    system_prompt_file: prompts/company_summary_system_v2.txt
    user_prompt_template_file: prompts/company_summary_user_template_v2.txt
    output_schema_file: prompts/company_summary_output_schema_v2.json
    prompt_manifest_file: prompts/company_summary_prompt_manifest_v2.yaml
...
- [ ] `company_summary_system_v2.txt` is loaded by the production LLM path.
- [ ] `company_summary_user_template_v2.txt` is loaded by the production LLM
      path.
- [ ] `company_summary_output_schema_v2.json` is loaded and validated.
- [ ] `company_summary_prompt_manifest_v2.yaml` is loaded and validated.
...
- [ ] Deterministic applicability mappings and scores match the Version 1.14
      regression baseline.
...
There is no default “skip completed companies” policy in Version 1.15.
...
`--reprocess-existing-ratings` SHALL NOT be used in Version 1.15.
...
selected `Y` row is the default Version 1.15 behavior.
...
## 40. Version 1.15 Runtime, Model, Domain, and Regression Controls

### 40.1 Authoritative Production Models

Version 1.15 defines the following production models:
...
The active Version 1.15 models remain:
...
## 42. Version 1.15 Directory, Workbook, and Cycle-Selection Contract

### 42.1 Authoritative Changes

Version 1.15 makes these three changes authoritative:
...
`Companies.xlsx`; legacy binary `.xls` format is not part of Version 1.15.

---

## 43. Version 1.15 Model Assignment Contract

### 43.1 Authoritative Model Assignments

Version 1.15 fixes the production model assignments as follows:
...
`qwen3:8b` is not part of the Version 1.15 production design.
...
### 44.1 Root Cause Addressed by Version 1.15
...
Version 1.15 treats this condition as an incomplete implementation, not a
...
OCR is not mandatory for Version 1.15, but silent ingestion of an empty
...
### 45.1 Root Cause Addressed by Version 1.15
...
Version 1.15 defines this as an incomplete search implementation.
...
Version 1.15 SHALL not introduce a minimum accepted-evidence-count gate.
...
## 46. Version 1.15 Cost-Free Search Configuration and Quota Enforcement
...
The following are conservative software caps for the Version 1.15
configuration:
...
CODER has completed the Version 1.15 search configuration only when:
...
## 47. Version 1.15 Gemma Runtime and Assessment-Report Contract
...
Version 1.15 addresses the narrative path without changing the authority of the
deterministic assessment.
...
CODER has completed Version 1.15 only when:
...
## 48. Version 1.15 Provider Credentials and PDF Presentation Contract
...
## 49. Version 1.15 dSPACE Product-Ownership and Narrative-Safety Contract
...
Regression tests SHALL compare representative Version 1.14 and Version 1.15
