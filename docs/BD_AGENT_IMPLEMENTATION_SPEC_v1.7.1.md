...
**Repository:** `BD-Agent-for-Power_Electronics`  
...
This document is the authoritative implementation specification for the repository `BD-Agent-for-Power_Electronics`.

Codex SHALL:

1. Preserve the repository name `BD-Agent-for-Power_Electronics` and Python package name `bd_agent_neural_net_coder`.
...
- a benchmark-selected, locally hosted, quantized 8–12B-class model for final summary generation;
- BGE embeddings through Ollama or a compatible local embedding runtime; CPU execution is the baseline, while OpenVINO acceleration MAY be enabled only after benchmark validation;
...
```text
BD-Agent-for-Power_Electronics/
├── AGENTS.md
├── README.md
├── pyproject.toml
├── docs/
...
### REQ-RUN-007 — Sequential MS.xlsx company registry orchestration

Version 1 SHALL support registry-driven batch execution from:
...
- select only rows whose normalized scan flag equals `Y`;
- skip rows with an existing rating by default;
- process selected companies sequentially;
- create one normal company run directory and run-registry record for each
  processed row;
- update the source workbook only after the corresponding deterministic
...
Open data/MS.xlsx
  ...
  -> reconcile valid pending_rating_updates.json entries when the workbook is writable
  -> preflight workbook write access
  -> normalize Scan Y/N and build the sequential company queue
  -> for each selected company:
       ...
       -> calculate deterministic Overall Product Match rating
       -> atomically write column H when MS.xlsx is writable,
          otherwise persist pending_rating_updates.json
       ...
       -> persist all logs, cache metadata, benchmark profile,
          workbook-update state, and run-registry records
  -> continue with the next company; no parallel company processing in Version 1
```
...
| `company_registry.yaml` | MS.xlsx path, worksheet and header mapping, selection policy, rating weights, bands, locking, backup, and atomic update controls |
  ...
  "evidence": [
    {
      ...
      "llm": {
        "model": "benchmark-selected-local-summary-model",
...
```json
{
  ...
  "methodology": {
    ...
    "prompt_versions": {},
    "llm_model": "benchmark-selected-local-summary-model",
...
| `company_registry.py` | Read MS.xlsx, validate headers, normalize flags, select eligible rows, and create immutable registry entries |
...
| `batch_runner.py` | Sequential orchestration of eligible MS.xlsx rows and batch-level pending updates |
...
python -m bd_agent_neural_net_coder scan-registry --workbook data/MS.xlsx
python -m bd_agent_neural_net_coder scan-registry \
  --workbook data/MS.xlsx --reprocess-existing-ratings
...
- `data/MS.xlsx` can be validated and read by header name;
- only rows with normalized `Scan Y/N = Y` are selected;
- rows with an existing `Overall Product Match` are skipped by default;
- eligible companies are processed sequentially;
...
## 39. MS.xlsx Company Registry and Overall Product Match

### 39.1 Purpose

The BD-Agent SHALL use an Excel workbook as the authoritative registry of
...
The current workbook layout contains:

- column G: `Scan Y/N`;
- column H: `Overall Product Match`.

The current sample workbook contains four company rows with `Y` in column G and
blank values in column H. This observation is informative only; the
implementation SHALL support any valid number of company rows.
...
### 39.3 Company Selection Rule

Only rows whose scan flag normalizes to `Y` SHALL be eligible.
...
Blank values, `N`, and invalid values SHALL be skipped and logged.
...
Scan Y/N = Y
AND
Overall Product Match is empty
```

This prevents completed companies from being processed repeatedly.
...
company_registry:
  ...
  selection:
    ...
    process_only_empty_ratings: true
    reprocess_existing_ratings: false
  ...
  update:
    ...
    reconcile_pending_before_selection: true
    create_backup: true
    lock_workbook: true
    atomic_replace: true
...
When `reprocess_existing_ratings: true`, every row with normalized
`Scan Y/N = Y` SHALL be processed again and the existing rating SHALL be
replaced after successful deterministic assessment.
...
def should_process(
    ...
    *,
    process_only_empty_ratings: bool,
    reprocess_existing_ratings: bool,
) -> bool:
    if not company.scan_enabled:
        return False

    if reprocess_existing_ratings:
        return True

    if process_only_empty_ratings:
        return company.existing_rating is None
...
Validate workbook and required headers
      ↓
Reconcile pending rating updates when writable
      ↓
Preflight workbook write access
      ↓
Read and normalize company rows
      ↓
Filter Scan Y/N = Y
      ↓
Exclude existing ratings by default
      ↓
Create sequential processing queue
      ↓
Process one company completely
      ↓
Persist deterministic assessment
      ↓
Calculate and write rating
      ↓
Attempt Local LLM summary and render report
      ↓
Continue with the next eligible company
```

Parallel company processing SHALL be disabled in Version 1 because concurrent
workbook updates could conflict.

Each company SHALL retain its own:
...
Column H SHALL contain a deterministic integer rating from `0` to `100`.
...
| Blank | Not processed or processing failed |

The Local LLM SHALL NOT calculate or modify this rating.

Only dSPACE portfolio items that passed YAML hard-rule validation SHALL contribute.
...
If no eligible dSPACE portfolio item exists, its corresponding product-score contribution
SHALL be `0.0`. If only one eligible dSPACE portfolio item exists, the second-product
contribution SHALL be `0.0`.
...
Column H SHALL be updated only after the deterministic assessment has
successfully completed and been persisted.

Required sequence:

```text
Company selected from MS.xlsx
        ↓
Evidence collection completed
        ↓
Deterministic product assessment completed
        ↓
...
Rating calculated deterministically
        ↓
Column H updated atomically when writable
OR pending_rating_updates.json persisted
        ↓
...
An LLM timeout, crash or invalid response SHALL NOT prevent a valid
deterministic rating from being stored.

If evidence collection or deterministic assessment fails:

- Python SHALL NOT write `0`;
- Python SHALL leave the existing cell unchanged;
- Python SHALL record the failure in the run log.

A value of `0` SHALL mean a completed deterministic assessment with no
meaningful fit. It SHALL NOT represent a technical failure.

### 39.9 Atomic Workbook Update

The application SHALL NOT modify the source workbook incrementally or rewrite
a complete company row.
...
Acquire workbook lock
    ↓
Create run backup if not already created
    ↓
...
Write MS.tmp.xlsx
    ↓
Reopen and validate company_id and rating
    ↓
Atomically replace MS.xlsx
    ↓
...
- create a backup before the first workbook update of a batch run;
- use a workbook lock during every write;
...
```text
data/backups/MS_<BATCH_TIMESTAMP>.xlsx
```
...
Every successful update SHALL produce a structured log record.

```json
{
  "event": "company_rating_written",
  ...
  "previous_value": null,
  "new_value": 83,
  "assessment_run_id": "SIEMENS-ENERGY-20260718T143522Z",
  "timestamp": "2026-07-18T15:21:08Z"
}
```

Failed update attempts SHALL include:

- error type;
- workbook path;
- company ID;
- attempted value;
- whether the source workbook remained unchanged;
...
```text
MS.xlsx
    selects which companies are processed
    and stores the deterministic overall rating

YAML + Python
    determine product eligibility, product scores,
    hard-rule outcomes and the 0–100 rating

BGE + ChromaDB
    retrieve supporting product-document knowledge

Local LLM
    writes the readable company assessment

Python
    validates all results, updates MS.xlsx,
    renders reports and persists audit artifacts
```
...
- replace a deterministic product result;
- change column H.
...
| Existing rating and default policy | unchanged | row skipped and logged |
| Evidence collection failure | unchanged | company run failed |
| Deterministic assessment failure | unchanged | company run failed |
| Rating calculation failure | unchanged | company run failed |
| Atomic workbook update failure | unchanged | error logged; report generation MAY continue |
| Local LLM failure | deterministic rating retained | deterministic fallback report |
| Report rendering failure | deterministic rating retained | run marked incomplete or failed according to report policy |
| Completed no-fit assessment | write `0–19` as calculated | normal completion |

### 39.13 Recommended CLI

```bash
python -m bd_agent_neural_net_coder scan-registry \
  --workbook data/MS.xlsx

python -m bd_agent_neural_net_coder scan-registry \
  --workbook data/MS.xlsx \
  --reprocess-existing-ratings
```
...
- **REQ-REGISTRY-001:** The agent SHALL use `data/MS.xlsx` and worksheet
...
- **REQ-REGISTRY-003:** The current workbook version SHALL validate columns G
  and H for scan flag and rating.
- **REQ-REGISTRY-004:** Only normalized scan value `Y` SHALL be eligible.
- **REQ-REGISTRY-005:** Existing ratings SHALL be skipped by default.
- **REQ-REGISTRY-006:** Reprocessing existing ratings SHALL require explicit
  configuration or CLI selection.
- **REQ-REGISTRY-007:** Version 1 SHALL process companies sequentially.
- **REQ-RATING-001:** The overall rating SHALL be deterministic and in the
  integer range `0–100`.
- **REQ-RATING-002:** Only hard-rule-passed products SHALL contribute.
- **REQ-RATING-003:** The Local LLM SHALL not calculate or alter the rating.
- **REQ-RATING-004:** A technical failure SHALL leave the rating cell unchanged.
- **REQ-RATING-005:** A value of zero SHALL mean completed no-fit assessment,
  not technical failure.
- **REQ-WORKBOOK-001:** The rating SHALL be written only after
  `final_assessment.json` is persisted.
- **REQ-WORKBOOK-002:** The update SHALL modify only the rating cell identified
  by company ID.
...
- **REQ-WORKBOOK-004:** Formatting, formulas and unrelated cells SHALL be
  preserved.
- **REQ-WORKBOOK-005:** Every update attempt SHALL be auditable.

### 39.15 Acceptance Criteria

- [ ] `MS.xlsx` opens and worksheet `Companies` exists.
...
- [ ] Existing ratings are skipped by default.
- [ ] Reprocessing can be enabled explicitly.
- [ ] Selected companies are processed sequentially.
- [ ] Every selected company receives its own timestamped run directory.
- [ ] The deterministic rating is reproducible from saved assessment values.
- [ ] Only hard-rule-passed products contribute to the rating.
- [ ] The source workbook is unchanged when deterministic processing fails.
- [ ] A valid rating is written before the Local LLM call.
- [ ] An LLM failure does not remove or change the rating.
- [ ] Only the intended rating cell changes.
...
### 39.16 Workbook Preflight and Deferred Rating Updates
...
- the workbook can be opened for a safe write-and-replace operation;
- an existing application or operating-system lock prevents replacement;
- pending updates from an earlier batch exist.

If the workbook is readable but not writable, company assessments MAY continue.

After each successful deterministic assessment, Python SHALL write a pending
update record instead of losing the rating:
...
```json
{
  ...
  "created_at": "2026-07-18T15:21:08Z"
}
```
...
At batch start and batch end, the agent SHOULD attempt to reconcile pending
updates when the workbook is writable.

Reconciliation SHALL:

1. validate the associated final assessment and hash;
2. locate the company by `company_id`;
3. verify the company name;
4. verify that the pending rating is in `0–100`;
5. avoid overwriting a different newer rating unless reprocessing was explicit;
6. perform the normal backup, temporary-file validation, and atomic replacement;
7. mark the pending record as applied instead of deleting the audit history.

A company with a valid pending update SHALL be treated as already processed by
the default selection policy, preventing duplicate work while column H is still
blank.
...
- **REQ-WORKBOOK-006:** A write-lock or open Excel session SHALL NOT invalidate a
  completed deterministic assessment.
- **REQ-WORKBOOK-007:** A failed immediate workbook write SHALL create a
  validated pending update record.
- **REQ-WORKBOOK-008:** Pending updates SHALL be reconciled before default
  company selection where possible.
- **REQ-WORKBOOK-009:** Valid pending entries SHALL prevent duplicate processing
  by default.
...
## 40. Version 1.7 Runtime, Model, Domain, and Regression Controls

### 40.1 Configurable Local Model Profile

The production design SHALL not hard-code a 27B Local LLM.

Version 1.7 SHALL use a configurable, benchmark-selected, quantized
8–12B-class local model for company-summary generation.

Recommended configuration:

```yaml
hardware_profiles:
  active_profile: notebook_32gb_no_cuda

  profiles:
    notebook_32gb_no_cuda:
      system_ram_gb: 32
      discrete_cuda_gpu: false
      production_model_candidates:
        - model_profile_id: local_8b_q4
        - model_profile_id: local_9b_q4
        - model_profile_id: local_12b_q4
      maximum_loaded_model_memory_gb: 12
      maximum_parallel_llm_requests: 1
      python_process_priority: below_normal
      embedding_threads: 4

llm:
  company_summary:
    model_profile: benchmark_selected
    temperature: 0.2
    max_output_tokens: 1800
    timeout_seconds: 120
```

The exact model identifier SHALL be selected through a local benchmark and
stored in the effective run configuration.

Hardware-specific acceleration, including Intel GPU, NPU, oneAPI, SYCL, or
OpenVINO execution, SHALL remain optional. CPU execution is the mandatory
baseline. An accelerated backend SHALL be enabled only when benchmark results
show improved stability and runtime on the actual target machine.

### 40.2 Model Benchmark

The repository SHALL provide a model benchmark command:

```bash
python -m bd_agent_neural_net_coder benchmark-models \
  --hardware-profile notebook_32gb_no_cuda
```
...
- representative summary quality using a fixed evaluation dataset.

The benchmark SHALL write:

```text
output/model_benchmarks/<TIMESTAMP>/benchmark_results.json
```

A model SHALL not become the active production profile merely because it is
faster. It SHALL also pass schema, grounding, and recommendation-preservation
criteria.
...
- arbitrary advanced Excel features are not guaranteed outside the controlled
  `MS.xlsx` registry template.

### 40.9 Normative Requirements

- **REQ-MODEL-001:** The production Local LLM SHALL be selected from a
  configurable 8–12B-class quantized model profile through benchmark evidence.
- **REQ-MODEL-002:** CPU inference SHALL remain the supported baseline.
- **REQ-MODEL-003:** Model benchmark results SHALL be persisted and versioned.
...
- [ ] Pending updates prevent duplicate default processing.
